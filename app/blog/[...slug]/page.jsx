...
const page = async ({ params, searchParams }) => {
  ...
  const dataCategorisAndFirstpost = await getData(`wp-json/okhub/v1/get-list-cat-and-first-posts?per_page=4`)
  const CateCurrent = dataCategorisAndFirstpost?.categories.find((c) => c.slug === params.slug[0])
  const dataGetAllPostsByCategories = await getData(`wp-json/okhub/v1/get-posts-by-category/1?cat_id=${CateCurrent?.id}&page=1&posts_per_page=4`)
